...
from emp 
where 10 = deptno; -- deptno = 10이여도 가능
...
from emp
where deptno in(10, 20) -- deptno = 10 or 20 은 안됨. 
...
select job, ename, sal
from emp
where job = 'CLERK' or job = 'MANAGER' -- where job in('CLERK', 'MANAGER')
order by job, sal;
...
-- 숫자 타입뿐만 아니아 문자열, 날짜 타입들도 대소비교(>, <, >=, <=)가 가능.
